import { BedrockRuntimeClientConfig, ContentBlock, Message, SystemContentBlock } from "@aws-sdk/client-bedrock-runtime"
...
import { CreditGrant, StringifiedCreditGrant } from "../../shared/ExtensionMessage"
...
export interface AiApiStop {
	refundType?: "full" | "partial"
	refundAmountBips?: number
	totalActionQuantityBips?: number
	activeCreditGrants?: CreditGrant[]
}

export interface StringifiedAiApiStop {
	refundType?: string
	refundAmountBips?: string
	totalActionQuantityBips?: string
	activeCreditGrants?: StringifiedCreditGrant[]
}

// Define types for stream events based on AWS SDK
export interface StreamEvent {
	aiApiStop?: StringifiedAiApiStop
	messageStart?: {
		role?: string
	}
	messageStop?: {
...
export class AiApiHandler extends BaseProvider implements SingleCompletionHandler {
	protected options: ProviderSettings
	private client: typeof aiApiClient
	private arnInfo: any
	...
	constructor(options: ProviderSettings) {
		...
		if (!this.options.apiModelId) {
			this.options.apiModelId = aiApiDefaultModelId
		}
		...
		this.client = aiApiClient
	}

	// Helper to guess model info from custom modelId string if not in aiApiModels
	...
	override async *createMessage(
		...
		// Note (sai): topP and temperature cannot be used together unless the model explicitly supports it
		// Claude 4.5 Sonnet does not support using topP and temperature together
		if (!thinkingEnabled && !modelConfig.id.includes("sonnet-4-5")) {
		...
		try {
			...
			const stream = this.client.stream(payload)
			if (!stream) {
			...
			for await (const chunk of stream) {
				...
				if (streamEvent.aiApiStop) {
					yield {
						type: "credits",
						refundType: streamEvent.aiApiStop?.refundType as AiApiStop["refundType"],
						refundAmountBips: Number.parseInt(streamEvent.aiApiStop?.refundAmountBips ?? "0"),
						totalActionQuantityBips: Number.parseInt(streamEvent.aiApiStop?.totalActionQuantityBips ?? "0"),
						activeCreditGrants: streamEvent.aiApiStop?.activeCreditGrants?.map(grant => ({
							uuid: grant.uuid,
							AccountUuid: grant.AccountUuid,
							category: grant.category as CreditGrant["category"],
							remainingQuantity: Number.parseFloat(grant.remainingQuantity),
	...
	// NOTE(kyle): completePrompt is for single completion requests like enhance prompt, generate
	// commit messages, generate terminal commands, etc. As of now, we're not supporting this. If
	// we need to support completePrompt later, we'll need to update the implementation to process
	// streaming messages from ai-api instead of all-in-one HTTP.
	async completePrompt(prompt: string): Promise<string> {
		throw new Error(`Not implemented, cannot complete ${prompt}`)
	}
	...
	private costModelConfig: { id: AiApiModelId | string; info: ModelInfo } = {
		id: "",
	...
	private parseArn(arn: string, region?: string) {
		...
		if (match && match[1] && match[3] && match[4]) {
			...
			if (originalModelId && result.modelId !== originalModelId) {
				...
				result.crossRegionInference = AiApiHandler.isSystemInferenceProfile(prefix)
			}
	...
	getModelById(modelId: string, modelType?: string): { id: AiApiModelId | string; info: ModelInfo } {
		// Try to find the model in aiApiModels
		const baseModelId = this.parseBaseModelId(modelId) as AiApiModelId

		let model
		if (baseModelId in aiApiModels) {
			//Do a deep copy of the model info so that later in the code the model id and maxTokens can be set.
			// The aiApiModels array is a constant and updating the model ID from the returned invokedModelID value
			// in a prompt router response isn't possible on the constant.
			model = { id: baseModelId, info: JSON.parse(JSON.stringify(aiApiModels[baseModelId])) }
		} else {
			// Use heuristics for model info, then allow overrides from ProviderSettings
			const guessed = this.guessModelInfoFromId(modelId)
			model = {
				id: aiApiDefaultModelId,
				info: {
					...JSON.parse(JSON.stringify(aiApiModels[aiApiDefaultModelId])),
					...guessed,
				},
			}
		}
	...
	override getModel(): {
		...
		const modelConfig = this.getModelById(this.options.apiModelId as string)
		modelConfig.id = `us.${modelConfig.id}`
	...
	private supportsAwsPromptCache(modelConfig: { id: AiApiModelId | string; info: ModelInfo }): boolean | undefined {
		// Check if the model supports prompt cache
		// The cachableFields property is not part of the ModelInfo type in schemas
		// but it's used in the aiApiModels object in shared/api.ts
		return (
...
- Model ID is correct for the requested features`,
	...
	private getErrorType(error: unknown): string {
		...
		for (const errorType of errorTypeOrder) {
			const definition = AiApiHandler.ERROR_TYPES[errorType]
	...
	private formatErrorMessage(error: unknown, errorType: string, _isStreamContext: boolean): string {
		const definition = AiApiHandler.ERROR_TYPES[errorType] || AiApiHandler.ERROR_TYPES.GENERIC
