...
/**
 * Checks for a file in .8thwallagent/launchPrompt.md and runs it immediately if it exists.
 */
export async function checkAndRunAutoLaunchingTask(context: vscode.ExtensionContext): Promise<void> {
	...
	const promptFilePath = vscode.Uri.joinPath(workspaceFolderUri, ".8thwallagent", "launchPrompt.md")
	...
	try {
		...
		vscode.commands.executeCommand("8th-wall-agent.newTask", { prompt })
	} catch (error) {
		if (error instanceof vscode.FileSystemError && error.code === "FileNotFound") {
			return // File not found, which is expected if no launchPrompt.md exists
		}
		console.error(`Error running .8thwallagent/launchPrompt.md: ${error}`)
	}
}
