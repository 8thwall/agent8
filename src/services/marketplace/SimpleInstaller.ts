...
export class SimpleInstaller {
	...
	private async installMode(
		...
		try {
			...
			if (error.code === "ENOENT") {
				...
				const fileName = target === "project" ? ".8thwallagentmodes" : "custom-modes.yaml"
				throw new Error(
	...
	private async installMcp(
		...
		try {
			...
			if (error.code === "ENOENT") {
				...
				const fileName = target === "project" ? ".8thwallagent/mcp.json" : "mcp-settings.json"
				throw new Error(
	...
	private async getModeFilePath(target: "project" | "global"): Promise<string> {
		if (target === "project") {
			...
			return path.join(workspaceFolder.uri.fsPath, ".8thwallagentmodes")
		} else {
	...
	private async getMcpFilePath(target: "project" | "global"): Promise<string> {
		if (target === "project") {
			...
			return path.join(workspaceFolder.uri.fsPath, ".8thwallagent", "mcp.json")
		} else {
			const globalSettingsPath = await ensureSettingsDirectoryExists(this.context)
