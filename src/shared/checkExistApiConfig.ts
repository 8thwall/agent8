...
export function checkExistKey(config: ProviderSettings | undefined) {
	...
	// Special case for human-relay, fake-ai, claude-code, gemini-cli, and ai-api providers which don't need any configuration.
	if (config.apiProvider && ["human-relay", "fake-ai", "claude-code", "gemini-cli", "ai-api"].includes(config.apiProvider)) {
		return true
	}
