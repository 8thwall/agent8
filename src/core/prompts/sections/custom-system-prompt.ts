...
export function getSystemPromptFilePath(cwd: string, mode: Mode): string {
	// kilocode_change
	return path.join(cwd, ".8thwallagent", `system-prompt-${mode}`)
}

/**
 * Loads custom system prompt from a file at .8thwallagent/system-prompt-[mode slug]
...
/**
 * Ensures the .8thwallagent directory exists, creating it if necessary
 */
export async function ensureRooDirectory(cwd: string): Promise<void> {
	// kilocode_change
	const rooDir = path.join(cwd, ".8thwallagent")
