...
describe("RooIgnore Response Formatting", () => {
	...
	describe("formatResponse.rooIgnoreError", () => {
		...
		it("should format error message for ignored files", () => {
			...
			expect(errorMessage).toContain(
				"Access to secrets/api-keys.json is blocked by the .8thwallagentignore file settings",
			)
			expect(errorMessage).toContain("continue in the task without using this file")
			expect(errorMessage).toContain("ask the user to update the .8thwallagentignore file")
		})
	...
	describe("getInstructions", () => {
		 ...
		 */
		it("should format .8thwallagentignore instructions for the LLM", async () => {
			// Create controller
			const controller = new RooIgnoreController(TEST_CWD)
			await controller.initialize()
		...
		it("should return undefined when no .8thwallagentignore exists", async () => {
			// Set up no .8thwallagentignore
			mockFileExists.mockResolvedValue(false)

			// Create controller without .8thwallagentignore
			const controller = new RooIgnoreController(TEST_CWD)
			await controller.initialize()
