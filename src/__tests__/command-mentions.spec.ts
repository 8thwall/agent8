	...
	describe("parseMentions with command support", () => {
		it("should parse command mentions and include content", async () => {
			...
			mockGetCommand.mockResolvedValue({
				...
				source: "project",
				filePath: "/project/.8thwallagent/commands/setup.md",
			})
		...
		it("should handle multiple commands in message", async () => {
			...
			mockGetCommand
				...
				.mockResolvedValueOnce({
					...
					source: "project",
					filePath: "/project/.8thwallagent/commands/deploy.md",
				})
				.mockResolvedValueOnce({
					name: "setup",
					content: setupContent,
		...
		it("should handle command loading errors during processing", async () => {
			...
			mockGetCommand.mockResolvedValue({
				...
				source: "project",
				filePath: "/project/.8thwallagent/commands/error-command.md",
			})
		...
		it("should handle command names with hyphens and underscores at start", async () => {
			mockGetCommand.mockResolvedValue({
				...
				content: "# Dev setup",
				source: "project",
				filePath: "/project/.8thwallagent/commands/setup-dev.md",
			})
...
> **Note**: This is important!`
			...
			mockGetCommand.mockResolvedValue({
				...
				content: commandContent,
				source: "project",
				filePath: "/project/.8thwallagent/commands/complex.md",
			})
		...
		it("should handle empty command content", async () => {
			mockGetCommand.mockResolvedValue({
				...
				content: "",
				source: "project",
				filePath: "/project/.8thwallagent/commands/empty.md",
			})
	...
	describe("command mention text transformation", () => {
		...
		it("should match commands anywhere with proper word boundaries", async () => {
			mockGetCommand.mockResolvedValue({
				...
				source: "project",
				filePath: "/project/.8thwallagent/commands/build.md",
			})
