// This file is copied from https://github.com/langchain-ai/langchainjs/blob/main/libs/langchain-aws/src/common.ts
// We need this file here because @langchain/aws does not export these helper functions
// however since Realtime API is currently a wrapper around Bedrock SDK we need the exact same functions to convert from bedrock to langchain and vice versa.
// TODO (sai): Remove the file when we move away from langchain.
...
function convertAIMessageToConverseMessage(msg: AIMessage): BedrockMessage {
  ...
  // biome-ignore lint/complexity/useOptionalChain: Ignore type errors in this file
    ...
    if ('redactedContent' in block) {
      ...
      const prevRedactedContent = (
        'redactedContent' in concatenatedBlock
          ? // biome-ignore lint/style/noNonNullAssertion: Ignore type errors in this file
